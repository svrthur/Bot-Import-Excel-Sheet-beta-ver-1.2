...
1.  Откройте вашу Google Таблицу. Ссылка: `https://docs.google.com/spreadsheets/d/17VeQQWTGotofrpNbUHDhUFhCc3qjLdwoesTxDDfJ7h4/edit`
...
## 4. Автоматическое скрытие завершенных кампаний

Бот теперь включает функцию автоматического скрытия строк (начиная со 120-й), которые имеют статус **"завершено"** в столбце **E**.

1.  **Как это работает**: Скрипт проверяет статус в колонке E и дату в колонке I (Дата окончания). Если с момента даты окончания прошло более 24 часов, строка скрывается.
2.  **Как включить**: 
    *   В редакторе Google Apps Script нажмите на иконку часов слева (**Триггеры**).
    *   Нажмите **Добавить триггер**.
    *   Выберите функцию: `hideFinishedCampaigns`.
    *   Выберите источник мероприятия: **По времени**.
    *   Выберите тип триггера: **По дням** или **По часам** (например, раз в час).
    *   Нажмите **Сохранить**.
...
*Инструкция подготовлена Replit Agent.*
